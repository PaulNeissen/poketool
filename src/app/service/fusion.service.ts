...
export class FusionService {
  ...
  getTypes(pokemon) {
    if (this.swapTypePokemons.includes(pokemon.id)) { // Swapped types
      return pokemon.types.slice().reverse(); 
    } else if (pokemon.id in this.dominantTypeMap) { // Dominant type
      return [this.dominantTypeMap[pokemon.id]];
    } else if (pokemon.types.length > 1 && pokemon.types[0] == 0 && pokemon.types[1] == 2) { // Dominant type for Normal/Flying
      return [2]; // Flying type
    } else {
      return pokemon.types.slice();
    }
  }
  ...
  fuse(head, body) {
    ...
    // LEGENDARY
    fusion.legendary = head.legendary || body.legendary;
  ...
  initFusions() {
    if (this.fusions.length == 0) {
      let lastIds = this.fusablePokemons.map(x => x[1]).filter(x => x > 251);
      console.log('LAST IDS', lastIds);
      ...
      for(let i = 0; i < 540; i++) {
        let tmp = this.pokemonService.pokemons.filter(x => x.evolveChain == i && (x.id <= 251 || lastIds.includes(x.id)));
        if (tmp.length > 0) {
          candidates.push(tmp.reduce((max, curr) => max.total > curr.total ? max : curr));
        }
      }

      candidates.forEach(head => {
        candidates.forEach(body => {
          this.fusions.push(this.fuse(head, body));
        });
      });
    }
  }
